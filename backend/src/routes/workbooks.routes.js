...
workbooksRouter.get(
  ...
  asyncHandler(async (req, res) => {
    res.json(await workbookRepo.listWorkbooks());
  }),
);
...
workbooksRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const w = await workbookRepo.getWorkbook(id);
...
workbooksRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const w = await workbookRepo.updateWorkbook(id, fields);
    if (!w) return res.status(404).json({ error: "Not found" });
    res.json(w);
  }),
);
...
workbooksRouter.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!(await workbookRepo.deleteWorkbook(id))) return res.status(404).json({ error: "Not found" });
    res.status(204).end();
  }),
);
...
workbooksRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const w = await workbookRepo.duplicateWorkbook(id);
    if (!w) return res.status(404).json({ error: "Not found" });
    res.status(201).json(w);
  }),
);
